...
namespace BadSample.Akiyama {
    public class DiscountManager {
        ...
        /// <param name="memberRank">顧客のランク</param>
        ...
        public static decimal ApplyDiscount(decimal price, MemberRank memberRank, int years) {
            if (memberRank == MemberRank.NonMember)
                return price;
            return price * (1 - RankDiscountRatio(memberRank)) * (1 - YearDiscountRatio(years));
        }
        ...
        /// <returns></returns>
        static decimal RankDiscountRatio(MemberRank rank) {
            switch (rank) {
        ...
        static decimal YearDiscountRatio(int years) {
